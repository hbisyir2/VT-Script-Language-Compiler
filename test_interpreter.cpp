...
TEST_CASE("test embedded if statements", "[interpreter]") {
	{
		std::string program = "(begin (define a (if True 10 100)))";
		Expression result = run(program);
		REQUIRE(result == Expression(10.));
	}
	{
		std::string program = "(if (define a (if True True False)) 20 50)";
		Expression result = run(program);
		REQUIRE(result == Expression(20.));
	}
	{
		std::string program = "(* 2 (if True 5 6) 10)";
		Expression result = run(program);
		REQUIRE(result == Expression(100.));
	}
	{
		std::string program = "(= (if True 1 2) 1)";
		Expression result = run(program);
		REQUIRE(result == Expression(true));
	}
	{
		std::string program = "(= (if False 1 2) 1)";
		Expression result = run(program);
		REQUIRE(result == Expression(false));
	}
}

TEST_CASE("test embedded defines", "[interpreter]") {
	{
		std::string program = "(begin (begin (if True 10 100)))";
		Expression result = run(program);
		REQUIRE(result == Expression(10.));
	}
	{
		std::string program = "(* 2 (define c (+ 5 6)) 10)";
		Expression result = run(program);
		REQUIRE(result == Expression(220.));
	}
	{
		std::string program = "(= (begin True 1 2) 1)";
		Expression result = run(program);
		REQUIRE(result == Expression(false));
	}
	{
		std::string program = "(= (begin False 2 1) 1)";
		Expression result = run(program);
		REQUIRE(result == Expression(true));
	}
}